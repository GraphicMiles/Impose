...
(function () {
  ...
  function createHarness() {
    ...
    /* One researched answer: search, then complete with the evidence.
       deps: query, search(q, limit), complete(system, user, onDelta),
       emit(event), onDelta(chunk), signal (optional AbortSignal). */
    function runAgent(deps) {
      var tool;
      try { tool = resolve("search"); }
      catch (e) { return Promise.reject(e); }
      deps.emit({ t: "status", text: "Searching the web" });
      return tool.run({ query: deps.query, limit: 8 }, { search: deps.search, emit: deps.emit }).then(function (out) {
        ...
        var system = "You are Nova, a helpful assistant. Answer ONLY from the evidence below. " +
          "Cite sources by number like [1]. If the evidence is thin, say so plainly.";
        return deps.complete(system, deps.query + "\n\nEvidence:\n" + lines, deps.onDelta).then(function () {
